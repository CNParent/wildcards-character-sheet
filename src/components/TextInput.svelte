...
{#if active}
    {#if label}
    <span class="align-self-center text-right mr-1 py-2 font-weight-bold" style="width: 8.0em; height: 2.5em;">{label}</span>
    {/if}
    <input bind:this={control} class="flex-grow-1 form-control" bind:value={content} on:blur={() => active = false}>
{:else}
    {#if label}
    <span class="align-self-center text-right border-right pr-1 py-2 font-weight-bold" style="width: 8.0em;">{label}</span>
    {/if}
